...
Item {
  ...
  // jackz fix 2026-08-16: these three were `required property` in the stock
  // omarchy.bar this was cloned from. That's fine for the *default* bar,
  // which the host builds via an inline Component with the values bound as
  // literal QML properties (set synchronously at construction). Third-party
  // bars like this one go through shell.qml's `pluginBarLoader`, an
  // asynchronous `Loader{ source: url }` that only assigns these via
  // `shell.configureBar(item, ...)` in `onLoaded` — i.e. AFTER the item has
  // already finished constructing. `required property` demands the value at
  // construction time, so the Loader's item fails required-property
  // validation, the Loader reports Loader.Error, and shell.qml's own
  // recovery path for that (`onStatusChanged`, meant to fall back to
  // omarchy.bar) throws `ReferenceError: errorString is not defined` and
  // never runs — so instead of falling back, the bar just doesn't render at
  // all, on any screen. Plain (non-required) properties are still filled in
  // fine by configureBar()'s post-construction assignment; only the
  // synchronous `required` contract is the problem here.
  ...
  // jackz customization 2026-08-16: single-bar-when-docked. Stock Bar.qml
  // puts one BarPanel on every Quickshell.screens entry, so plugging in the
  // Studio Display produced two live bars (laptop panel + external) at once.
  // Jack wants exactly one: the laptop panel alone when undocked, the
  // external display alone once it's present. Matches by output name rather
  // than screen count so it stays correct if a screen is ever added that
  // isn't the laptop panel — only the hardcoded laptop output name is ever
  // excluded, and only when something else is also connected.
  readonly property string laptopScreenName: "eDP-1"
  readonly property var activeScreens: {
    var screens = Quickshell.screens || []
    if (screens.length <= 1) return screens
    var external = screens.filter(function(s) { return s && s.name !== root.laptopScreenName })
    return external.length > 0 ? external : screens
  }
  ...
  Variants {
    // jackz customization: root.activeScreens instead of Quickshell.screens
    // (single-bar-when-docked, see the property comment near the top).
