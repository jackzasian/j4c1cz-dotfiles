...
def main() -> int:
    ...
    # Nerd Font glyph rather than a colour emoji, so the label matches the
    # weight and hue of every other icon in the bar.
    ICON = ""
    ...
    if not rows:
        ...
        tip = "\n".join(lines)
        # The Quattro bar paints only class "active" in the urgent colour, so
        # reserve it for a high-priority item due today (waybar used to style
        # every class via CSS, which is why this used to be inverted).
        cls = "active" if high_today else "scheduled"
